...
class MaxEnt(object):
    ...
    def compute_prob_pwl(self, optobj):
        ...
        for i, tmp in enumerate(all_perms):
            vec = np.asarray(tmp)
            p_vec = optobj.prob_dist_pwl(vec)*self.maxent_prob[i]

            self.total_prob += p_vec
            self.maxent_prob[i] = p_vec
    ...
    def main(self, file_num=None):
        ...
        all_p = opt.compute_all_prob(self.num_feats)
        print(all_p)
        ...
        if len(two_wayc)!=0:
            for k,v in two_wayc.items():
                feats = ExtractFeatures(cleaneddata.values)
                feats.set_two_way_constraints({k:v})
                feats.set_three_way_constraints({})
                feats.set_four_way_constraints({})
                ...
                print(self.maxent_prob)
        
        if len(three_wayc)!=0:
            for k,v in three_wayc.items():
                feats = ExtractFeatures(cleaneddata.values)
                feats.set_two_way_constraints({k:v})
                feats.set_three_way_constraints({})
                feats.set_four_way_constraints({})
        ...
        self.maxent_prob = self.maxent_prob/self.total_prob
        print('P vectors are', self.maxent_prob)
        print('total_prob is', self.total_prob)
