...
def read_perturb_prob_exp(filename):
	with open(filename, "rb") as outfile:
		prob = pickle.load(outfile,encoding='latin1')
...
def compute_kl(i, size, num_feats, perturb_prob):
	...
	perturb_exp_file = '../output/d'+str(size)+'_'+str(num_feats)+'_perturb/syn_maxent_expt'+str(i)+'_'+str(perturb_prob)+'.pickle'
	m_dist_exp, m_prob_exp = read_perturb_prob_exp(perturb_exp_file)
	...
	p2 = np.array(m_dist_p)
	q2 = np.array(m_dist_exp)
	kl_div.append(kl_divergence(q2,p2))

	return true_prob, m_prob_p, m_prob_r, m_prob_exp, kl_div
...
def plot(file_num, num_feats, size):
	...
	for p in p_prob:
		t, p, rb, re, k = compute_kl(file_num, size, num_feats, p)
		true.append(t)
		perturb.append(p)
		reg_box.append(rb)
		reg_exp.append(re)
	...
	for num,i in enumerate(lst):
		...
		reg_exp[num] = np.around(reg_exp[num], decimals=4)
		...
		if best_w == len(kl[num])-1:
			i.plot(xvec, reg_exp[num][0], label='Exp. Prior, KL Div: '+str(np.around(kl[num][len(kl[num])-1], decimals=4)))
		else:
			i.plot(xvec, reg_exp[num][0], linestyle='dotted', label='Exp. Prior. , KL Div: '+str(np.around(kl[num][len(kl[num])-1], decimals=4)))
		for w in range(len(width)):
			if w == best_w:
				i.plot(xvec, reg_box[num][w], label='Width = '+str(width[w])+' , KL Div: '+str(np.around(kl[num][w], decimals=4)))
			else:
				i.plot(xvec, reg_box[num][w], linestyle='dotted', label='Width = '+str(width[w])+' , KL Div: '+str(np.around(kl[num][w], decimals=4)))
