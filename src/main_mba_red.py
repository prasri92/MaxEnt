...
outfilename = '../out/pickles/obj_red_mba_all_k_fillNA.pk'
...
print "Writing the object file"
with open(outfilename, "wb") as outfile:
    pickle.dump(opt, outfile, protocol=pickle.HIGHEST_PROTOCOL)


def iter_nCk_binvect(n, k):
    for bits in itertools.combinations(range(n), k):e
        tmp = np.zeros(n)
#         print bits
        tmp[list(bits)] = 1
        yield tmp
...
up_limit = 6
num_vals = np.arange(up_limit)  # will have values up to up_limit - 1
prob_vals = np.zeros(len(num_vals))

for i, val in enumerate(num_vals):
    prob_vals[i] = compute_prob_exact(opt, val)

np_filename = 'numpy.npy'
np.save(np_filename, prob_vals)
