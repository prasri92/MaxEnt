...
class Optimizer(object):
    ...
    def compute_data_stats(self, partition):
        ...
        N = self.feats_obj.N        #number of patient vectors (rows)
        data_arr = self.feats_obj.data_arr        #each array
        for i in range(N):
        # for i in range(1, N):
            rvec = data_arr[i, partition] #take only diseases in partition
            # print("RVEC", rvec)
            tmp_arr = self.util_compute_array(rvec, partition, twoway_dict, 
                                    threeway_dict, fourway_dict, findpos,
                                    num_feats, num_2wayc, num_3wayc, num_4wayc) #gets some stats
    ...
    def solver_optimize(self):
        ...
        for i, partition in enumerate(parts):
            ...
            if len(partition) == 1:     # just use the MLE          
                ...
                datavec_partition = self.compute_data_stats(partition) #this looks at N_r variable for every vector
                c_matrix_partition = self.util_constraint_matrix(partition) #change (for only non zero vectors)
