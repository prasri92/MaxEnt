...
class Optimizer(object):
    ...
    `l_bfgs_b` function from scipy for finding an optimal set of params.
    Attributes:
    ...
    def solver_optimize(self):
        ...
        for i, partition in enumerate(parts):
            ...
            if len(partition) == 1:     # just use the MLE          
                ...
                # All thetas (alpha + betas are included as one single vector)
                len_theta = datavec_partition.shape[0]*2
                a = np.random.RandomState(seed=1)
                initial_val = a.rand(len_theta)
                # calc support from which we can obtain the 95% CI for the width 
                f = datavec_partition/self.feats_obj.N 
                ci = np.sqrt(f*(1-f)/self.feats_obj.N)*1.96 # 1.96 is the z-score for the 95% CI 
                upper_bound = f+ci
                lower_bound = f-ci
                print("Support: ", f, "\nUpper bound:", f+ci, "\nLower bound:", f-ci)
                ...
                def func_objective(thetas):
                    ...
                    a_term = np.dot(thetas_a, upper_bound)
                    b_term = np.dot(thetas_a, lower_bound)

                    norm_term = -1 * N * self.log_norm_Z((thetas_a-thetas_b), partition, c_matrix_partition)
                    objective_sum = theta_term_a - theta_term_b + norm_term - a_term - b_term

                    return (-1 * objective_sum) # SINCE MINIMIZING IN THE LBFGS SCIPY FUNCTION

                
                bnds = Bounds(lb=0, ub=np.inf)
                optimThetas = minimize(func_objective, x0=initial_val, method='L-BFGS-B', bounds=bnds,
                    options={'disp':False, 'maxcor':20, 'ftol':2.2e-10, 'maxfun':1000000})
                                
                opt_a, opt_b = np.split(optimThetas.x, 2)
                ...
                solution[i] = opt_a-opt_b
                
                with np.errstate(over='raise', invalid='raise'):
                    try:
                        norm_sol[i] = np.exp(self.log_norm_Z(opt_a-opt_b, partition, c_matrix_partition))
                        inn_arr = np.dot(c_matrix_partition, opt_a-opt_b)
                        ...
                        print('Before exp, norm_z is: ', self.log_norm_Z(opt_a-opt_b, partition, c_matrix_partition))
                        print('Z(theta): ', norm_sol[i])
