...
def compute_prob(i, j=None):
	exact = '../output/d250_10/syn_maxent_expt'+str(i)+'Mu'+str(10)+'.pickle'
	approximated = '../output/d250_10/syn_maxent_expt'+str(i)+'Mu'+str(2)+'.pickle'
	p = (np.array(read_prob_dist(exact)))
	q = (np.array(read_prob_dist(approximated)))
	approx_maxent_prob = read_true_prob(approximated)
	exact_maxent_prob = read_true_prob(exact)           
	kl_div = kl_divergence(q,p)
	true_prob = read_true_prob(exact)
	return approx_maxent_prob, exact_maxent_prob, kl_div, true_prob
...
if __name__ == '__main__':
    ...
    for i in range(3, 24, 10):
        ...
        approx_maxent_prob.append(m)
        exact_maxent_prob.append(e)
        kl.append(k)
        true_prob.append(t)
	...
	for num,i in enumerate(lst):
	   	print(approx_maxent_prob[num])
		i.plot(xvec, approx_maxent_prob[num], 'r', label='Maxent')
		i.plot(xvec, exact_maxent_prob[num], 'b', label='Empirical')
		i.axis(plot_lims)
		i.set_xticks(x_ticks)
		# for xy in zip(xvec, approx_maxent_prob[num]):
    ...
    for num,i in enumerate(lst):
        row = [num+1]
        for prob in approx_maxent_prob[num]:
            row.append(prob)
            out.writerow(row)
        print('xvec', xvec)    
        print('Maxent prob',approx_maxent_prob[num])
        print('Empirical prob', exact_maxent_prob[num])
        i.plot(xvec, approx_maxent_prob[num] , 'r', label='Approximate')
        i.plot(xvec, exact_maxent_prob[num], 'b', label='Exact')
    ...
    fig.suptitle('Diseases = 10, Dataset Size = 250, Mu=2', y=0.99, fontsize=10)
